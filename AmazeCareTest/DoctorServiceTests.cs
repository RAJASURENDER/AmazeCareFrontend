...
namespace AmazeCareTest
{
    ...
    public class DoctorServiceTests
    {
        ...
        [Test]
        public async Task UpdateDoctorExperience_NonExistingId_ReturnsNull()
        {
            // Arrange
            int nonExistingId = 999;
            float newExperience = 5.0f;

            var repositoryMock = new Mock<IRepository<int, Doctors>>();
            repositoryMock.Setup(repo => repo.GetAsync(nonExistingId)).ReturnsAsync((Doctors)null);

            var service = new DoctorService(repositoryMock.Object);

            // Act
            var result = await service.UpdateDoctorExperience(nonExistingId, newExperience);
        ...
        [Test]
        public async Task UpdateDoctorQualification_NonExistingId_ReturnsNull()
        {
            // Arrange
            int nonExistingId = 999;
            string newQualification = "MD";
            ...
            // Act
            var result = await service.UpdateDoctorQualification(nonExistingId, newQualification);
        ...
        [Test]
        public void UpdateDoctor_NonExistingDoctor_ThrowsNoSuchDoctorException()
        {
            // Arrange
            var nonExistingDoctor = new Doctors { DoctorId = 999 };

            var repositoryMock = new Mock<IRepository<int, Doctors>>();
            repositoryMock.Setup(repo => repo.GetAsync(nonExistingDoctor.DoctorId)).ReturnsAsync((Doctors)null);

            var service = new DoctorService(repositoryMock.Object);

            // Act & Assert
            Assert.ThrowsAsync<NoSuchDoctorException>(async () => await service.UpdateDoctor(nonExistingDoctor));
        }
